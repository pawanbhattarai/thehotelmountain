...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Network printer connectivity test (for PWA direct printing)
  app.post("/api/printer-configurations/test-network", async (req: any, res) => {
    try {
      ...
      const result = await new Promise((resolve) => {
        ...
        socket.connect(port, ipAddress, () => {
          clearTimeout(timeout);
          socket.destroy();
          resolve({ success: true, message: "Network printer accessible" });
        });
  ...
  // Direct print to network printer (for PWA)
  app.post("/api/printer-configurations/print", async (req: any, res) => {
    try {
      const { ipAddress, port = 9100, data } = req.body;
      
      if (!ipAddress || !data) {
        return res.status(400).json({ success: false, error: "IP address and print data required" });
      }
